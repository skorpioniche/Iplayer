...
namespace IntellectualPlayer.Processing.Search
{
    public sealed class AlgorithmEstimator
    {
        ...
        public AlgorithmEstimator(PlayerCore core, string hashMapPath, string scoresPath)
            : this(core, hashMapPath, scoresPath, DefaultRankToScore)
        {
        }

        public AlgorithmEstimator(PlayerCore core, string hashMapPath, string scoresPath, Func<int, int, int> transformRankToScore)
        {
            if (core == null)
            {
